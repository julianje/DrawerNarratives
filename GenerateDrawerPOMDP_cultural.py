...
import csv
import numpy as np
# Differences from rational POMDP:
# Has a left-right bias
# Has a horizontal over vertical bias
# 
		...
		if R[index][0] == 'Take':
			...
			if ExecState != 'Death' and FinalState != "Death":
				...
				if R[index][0][1:4] == R[index][2][1:4]:
					...
					if Startcoords[1] == DrawerDimensions[1]-1: # This means that you're in the right row.
						# Now check if your next state is at the beginning:
						if Endcoords[1] == 0 and Endcoords[0] == Startcoords[0] + 1:
							#sys.stdout.write(str(Startcoords)+","+str(Endcoords)+"\n")
							Cost = 0
					# Now compute the cost:
					# CHANGE 2: WEIGHT VERTICAL MOVEMENT BY 1.5
...
def PrintPOMDP(POMDPList):
	...
	filewriter = open(TrialName+"_cultural.POMDP","w+")
